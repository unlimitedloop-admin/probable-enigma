...
| SS-004 | P0 | Ready | Make `BgStarField` load transactional | Invalid counts/values/truncation preserve the old star field |
...
| SS-007 | P0 | Blocked by SS-004 | Replace `BgStarField` randomness with a scripted pattern | Save/load and replay produce the same star sequence |
...
| SS-012 | P0 | In progress | Audit and remove nondeterministic random sources | No simulation/render path uses wall-clock seed, `rand()`, or `random_device` |
