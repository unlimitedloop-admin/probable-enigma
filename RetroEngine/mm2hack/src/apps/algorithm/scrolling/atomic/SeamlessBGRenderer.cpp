...
namespace mm2hack::apps::algorithm::scrolling::atomic
{
    ...
    {
        const auto& tiles = _cache.Tile(pageIndex, 0, 0); // 触っておく（無意味アクセスでもOK）
        // 16x15 ループで直接描画
        for (int ty = 0; ty < 15; ++ty)
        {
            for (int tx = 0; tx < 16; ++tx)
            {
                const int id = static_cast<int>(_cache.Tile(pageIndex, tx, ty));
                _bg.DrawTileById(_tileset, id, dstX + tx * _ts, dstY + ty * _ts);
            }
        }
    }
}