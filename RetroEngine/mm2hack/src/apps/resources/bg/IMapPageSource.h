...
namespace mm2hack::apps::resources::bg
{
    ...
    public:
        ...
        virtual std::optional<ScrollKind> ScrollTypeRight(std::size_t pageIndex) const = 0;
        // Get scroll type for each direction (returns std::nullopt if not scrollable)
        virtual std::optional<ScrollKind> ScrollTypeLeft(std::size_t pageIndex) const = 0;
        // Get scroll type for each direction (returns std::nullopt if not scrollable)
        virtual std::optional<ScrollKind> ScrollTypeUp(std::size_t pageIndex) const = 0;
        // Get scroll type for each direction (returns std::nullopt if not scrollable)
        virtual std::optional<ScrollKind> ScrollTypeDown(std::size_t pageIndex) const = 0;

        // Get neighboring page index for each direction (returns std::nullopt if no neighbor)
        virtual std::optional<std::size_t> NeighborRight(std::size_t pageIndex) const = 0;
        // Get neighboring page index for each direction (returns std::nullopt if no neighbor)
        virtual std::optional<std::size_t> NeighborLeft(std::size_t pageIndex) const = 0;
        // Get neighboring page index for each direction (returns std::nullopt if no neighbor)
        virtual std::optional<std::size_t> NeighborUp(std::size_t pageIndex) const = 0;
        // Get neighboring page index for each direction (returns std::nullopt if no neighbor)
        virtual std::optional<std::size_t> NeighborDown(std::size_t pageIndex) const = 0;

        // Map room id -> page index (returns nullopt if not found)
        virtual std::optional<std::size_t> RoomToPageIndex(uint8_t room) const = 0;
